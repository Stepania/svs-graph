...
namespace SVSModel
{
    class Fertiliser
    {
        //public static Dictionary<DateTime, double> CalculateOutputs(DateTime[] simDates, ref Dictionary<DateTime, double> soilN, Dictionary<string, object> config)
        //{
          /*  Dictionary<DateTime, double> fert = Functions.dictMaker(simDates, new double[simDates.Length]);
            //Apply Planting N
            fert[Functions.Date(config["CurrentEstablishDate"])] = Functions.Num(config["EstablishN"]);
            soilN[Functions.Date(config["CurrentEstablishDate"])] += Functions.Num(config["EstablishN"]);
